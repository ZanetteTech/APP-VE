            ...
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2 flex-wrap mt-1">
                   <p className="text-foreground font-medium">{vehicle.modelo}</p>
                   ...
                   {vehicle.tipo_entrada && (
                    <Badge variant="outline" className={`text-[10px] font-bold uppercase tracking-wider border ${getTipoBadgeStyle(vehicle.tipo_entrada)}`}>
                      {vehicle.tipo_entrada}
                    </Badge>
                   )}
                </div>
            ...
            <div className="absolute top-4 right-4">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  {isFeatureEnabled('vehicle_ver') && (
                    <DropdownMenuItem onClick={() => setShowDetailsDialog(true)}>
                      <Eye className="w-4 h-4 mr-2" />
                      VER
                    </DropdownMenuItem>
                  )}
                  {isFeatureEnabled('vehicle_pdf') && (
                    <DropdownMenuItem onClick={handlePDF}>
                      <FileText className="w-4 h-4 mr-2" />
                      PDF
                    </DropdownMenuItem>
                  )}
                  {hideActions && vehicle.status === 'entrada' && isFeatureEnabled('vehicle_solicitar') && (
                  ...
                  {!hideActions && (
                    <>
                      {vehicle.status === 'entrada' && onExit && isFeatureEnabled('vehicle_saida') && (
                        <DropdownMenuItem onClick={() => onExit(vehicle)} className="text-destructive focus:text-white focus:bg-destructive">
                          <LogOut className="w-4 h-4 mr-2" />
                          SAÍDA
                        </DropdownMenuItem>
                      )}
                      {vehicle.status === 'saida' && onRevert && isFeatureEnabled('vehicle_reverter') && (!currentUser?.id || currentUser.id === vehicle.user_id) && (
                        <DropdownMenuItem onClick={() => onRevert(vehicle)} className="text-orange-500 focus:text-white focus:bg-orange-500">
                          <RotateCcw className="w-4 h-4 mr-2" />
                          REVERTER
                        </DropdownMenuItem>
                      )}
                      {onEdit && isFeatureEnabled('vehicle_editar') && (
                        <DropdownMenuItem onClick={() => onEdit(vehicle)}>
                          <Edit className="w-4 h-4 mr-2" />
                          EDITAR
                        </DropdownMenuItem>
                      )}
                      {isFeatureEnabled('vehicle_whatsapp') && (
                        <DropdownMenuItem onClick={handleWhatsApp}>
                          <Share2 className="w-4 h-4 mr-2" />
                          WHATSAPP
                        </DropdownMenuItem>
                      )}
                      {onDelete && isFeatureEnabled('vehicle_excluir') && (
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-yellow-600">
              ...
              {currentUser?.role === 'CADASTRO' ? 'O QUE DESEJA FAZER?' : 'VEÍCULO SOLICITADO'}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded-lg border border-yellow-200 dark:border-yellow-800">
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                ...
                {currentUser?.role === 'CADASTRO' ? ' Escolha uma opção abaixo:' : ' Para acessar os dados ou realizar ações, é necessário confirmar sua senha.'}
              </p>
            </div>
            ...
            {currentUser?.role === 'CADASTRO' ? (
              <div className="flex flex-col gap-3 py-4">
                <Button 
                  ...
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold h-12 text-lg shadow-md transition-all hover:scale-[1.02]"
                >
                  <Eye className="w-5 h-5 mr-2" />
                  VISUALIZAR
                </Button>
